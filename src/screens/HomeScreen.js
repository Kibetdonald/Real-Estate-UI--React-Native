...
const HomeScreen = ({ navigation }) => {
  ...
  //   Function to pick the list options
  const ListOptions = () => {
    return (
  ...
  //   Start of the card that takes in the data from allData element
  const Card = ({ allData }) => {
    return (
      <Pressable
        activeOpacity={0.8}
        onPress={() => navigation.navigate("DetailsScreen", allData)}
      >
        <View style={style.card}>
          {/* allData image */}
          <Image source={allData.image} style={style.cardImage} />
    ...
    <SafeAreaView style={{ backgroundColor: COLORS.white, flex: 1 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <FlatList
          ...
          horizontal
          data={allData}
          renderItem={({ item }) => <Card allData={item} />}
        />
        <FlatList
          snapToInterval={width - 20}
          showsHorizontalScrollIndicator={false}
